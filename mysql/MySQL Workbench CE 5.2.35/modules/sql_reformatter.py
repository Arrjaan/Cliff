...
def flatten_comma_sep_node(node, sep = ""):
...
class SQLPrettifier:
    ...
    opt_always_quote_identifiers = True
    ...
    def run(self):
        return self.traverse(self.ast, [])[1]

    def traverse(self, ast, path):
        ...
        for node in children:
            processed_node = self.traverse(node, path + [symbol])
            if processed_node:
    ...
    def sym_function_call_keyword(self, node):
        flattened = flatten_comma_sep_node(node)
        if "select_derived_union" not in self.current_path:  # try to generate more compact output for nested joins
            try:
                if self.opt_func_arg_per_line or len(flattened) > self.opt_expr_length_per_line or "\n" in flattened:
                ...
                print node
        return flattened
    sym_function_call_conflict = sym_function_call_keyword
    sym_geometry_function = sym_function_call_keyword
    sym_function_call_nonkeyword = sym_function_call_keyword    
